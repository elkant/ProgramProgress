...
//  String unique=(String)session.getAttribute("countyid");
	dbConnect conn = new dbConnect();
 	...
 	// update the table with the value that was edited from the form
        if(CID == 1){
	 query1 = "update districts set DistrictName='"+value+"' where districtID='"+id+"'";
          conn.state.executeUpdate(query1);
//           out.println("Saved, Refresh page");
        }
        else if(CID== 2){
            query2 = "update districts set countyID='"+value+"' where districtID='"+id+"'";
...
//              out.println("Saved, Refresh page");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(UpdateDistrict.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
