...
//  String unique=(String)session.getAttribute("countyid");
	dbConnect conn = new dbConnect();
 	...
 	// update the table with the value that was edited from the form
        if(CID == 1){
	 query1 = "update baseline set QtargetMen='"+value+"' where baselineID='"+id+"'";
          conn.state.executeUpdate(query1);
//           out.println("Saved, Refresh page");
        }
        else if(CID== 2){
            query2 = "update baseline set QtargetWomen='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query2);
//              out.println("Saved, Refresh page");
        }
        else if(CID== 3){
            query3 = "update baseline set menBaseline='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query3);
//             out.println("Saved, Refresh page");
        }
        else if(CID== 4){
            query3 = "update baseline set womenBaseline='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query3);
//             out.println("Saved, Refresh page");
        }
        else if(CID==5){
             query4 = "update baseline set endTargetMen='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query4);
//              out.println("Saved, Refresh page");
            }
        else if(CID==6){
             query4 = "update baseline set endTargetWomen='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query4);
//              out.println("Saved, Refresh page");
            }
        else if(CID==7){
             query4 = "update baseline set FinancialYear='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query4);
//              out.println("Saved, Refresh page");
            }
        else if(CID==8){
             query4 = "update baseline set Quarter='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query4);
//              out.println("Saved, Refresh page");
            }
            else if(CID==9){
             query4 = "update baseline set District='"+value+"' where baselineID='"+id+"'";
             conn.state.executeUpdate(query4);
//              out.println("Saved, Refresh page");
            }
