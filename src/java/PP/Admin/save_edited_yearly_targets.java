 ...
 */
public class save_edited_yearly_targets extends HttpServlet {
HttpSession session;
...
for (int i=1; i<=total;i++){
  ...
  county_id=ct[0];
   ...
   String target_total=request.getParameter("target_total"+i);
    ...
    String chek_existence="SELECT COUNT(id) FROM yearly_targets WHERE county_id='"+county_id+"' && district_id='"+district_id+"' && indicator_id='"+indicator_id+"' && id!='"+data_id+"' && year='"+year+"'";
    ...
    if(found==0){
  String database_updator="UPDATE yearly_targets SET county_id='"+county_id+"', district_id='"+district_id+"',year='"+year+"',"
          + "target_men='"+tar_men+"',target_women='"+tar_women+"',target_combined='"+tar_total+"' WHERE id='"+data_id+"'"; 
...
response.sendRedirect("edit_yearly_targets.jsp");
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(save_edited_yearly_targets.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
