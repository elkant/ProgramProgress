 ...
 * @author Maureen
 */
public class UploadData_HIVNonCum extends HttpServlet {
...
//System.out.println("Computer name "+computername);
     ...
     String pth = "C:/PPT_UPLOADS/PPTDATA.xlsx";
       ...
       // Sheet sheet = wb.getSheetAt(0);
        Sheet sheet = wb.getSheet("HIV_Noncummulative");
...
//            System.out.println(" last cell no = "+lastCellNo);
            ...
            if (id1 > -1) {
                ...
                if (conn.rs3.next()) {
                    ...
                    query = "insert into indicatorachievedcombined(resultID,county,district,totalAchieved,reportingPeriod,financialYear,titleID)"
                            + "VALUES('"+uniqueid().trim()+"','" + county + "','" + district + "','" + id1 + "','" + quarter + "','" + year + "','12')";
                }
            ...
            if (id2 >-1) {
                ...
                if (conn.rs3.next()) {
                    query = "update indicatorachieved set menAchieved='" + id2 + "',womenAchieved='" + id3 + "' where resultID='" + conn.rs3.getString(1) + "'";
                    ...
                    query = "insert into indicatorachieved(resultID,county,district,menAchieved,womenAchieved,reportingPeriod,financialYear,titleID)"
                            + "VALUES('"+uniqueid().trim()+"','" + county + "','" + district + "','" + id2 + "','" + id3 + "','" + quarter + "','" + year + "','17')";
                }
                System.out.println(query);
                try {
                    conn.state.executeUpdate(query);
            ...
            if (id4 >-1) {
                ...
                if (conn.rs3.next()) {
                    ...
                    query = "insert into indicatorachieved(resultID,county,district,menAchieved,womenAchieved,reportingPeriod,financialYear,titleID)"
                            + "VALUES('"+uniqueid().trim()+"','" + county + "','" + district + "','" + id4 + "','" + id5 + "','" + quarter + "','" + year + "','18')";
                }
                System.out.println(query);
                try {
                    conn.state.executeUpdate(query);
            ...
            if (id6 >-1) {
                ...
                if (conn.rs3.next()) {
                    query = "update indicatorachieved set menAchieved='" + id6 + "',womenAchieved='" + id7 + "' where resultID='" + conn.rs3.getString(1) + "'";

                } else {
                    query = "insert into indicatorachieved(resultID,county,district,menAchieved,womenAchieved,reportingPeriod,financialYear,titleID)"
                            + "VALUES('"+uniqueid().trim()+"','" + county + "','" + district + "','" + id6 + "','" + id7 + "','" + quarter + "','" + year + "','19')";
                }
                System.out.println(query);
                try {
                    conn.state.executeUpdate(query);
            ...
            if (id8  >-1) {
                String query = "";
                String checkexistingval = "Select * from indicatorachieved where  district='" + district + "' and financialYear='" + year + "' and titleID='21' and reportingPeriod='" + quarter + "' ";
                conn.rs3 = conn.state3.executeQuery(checkexistingval);
                ...
                if (conn.rs3.next()) {
                    ...
                    query = "insert into indicatorachieved(resultID,county,district,menAchieved,womenAchieved,reportingPeriod,financialYear,titleID)"
                            + "VALUES('"+uniqueid().trim()+"','" + county + "','" + district + "','" + id8 + "','" + id9 + "','" + quarter + "','" + year + "','21')";
                }
//                              conn.state.executeUpdate(query);
                try {
                    conn.state.executeUpdate(query);
