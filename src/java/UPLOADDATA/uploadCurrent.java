 ...
 * @author Maureen
 */
public class uploadCurrent extends HttpServlet {
...
//        String pth = mydrive + ":/PPT_UPLOADS/" + itemName;
       ...
       //  new File(dbconnpath).mkdir();
        ...
        Sheet sheet = wb.getSheet("QryHIV - noncummulative");
//************************************** THIS UPLOADS THE HIV NON CUMMULATIVE SHEET*********************************************
...
//            System.out.println(" last cell no = "+lastCellNo);
           ...
           // System.out.println("County :"+county+"District :"+district+"quarter :"+quarter+"id1 :"+id1+"id2: "+id2+"id3 :"+id3+"id4 :"+id4+"id5 :"+id5+"id6 :"+id6+"id7 :"+id7+"Year :"+year);
              ...
              if(id1>-1){
String query="";                            
String checkexistingval="Select * from indicatorachievedcombined where  district='"+district+"' and financialYear='"+year+"' and titleID='65' and reportingPeriod='"+quarter+"' ";                              
...
if(conn.rs3.next()){
  ...
  query = "update indicatorachievedcombined set totalAchieved='"+id1+"' where resultID='"+conn.rs3.getString(1)+"'";
    ...
    query = "insert into indicatorachievedcombined(resultID,county,district,totalAchieved,reportingPeriod,financialYear,titleID)"
                                   + "VALUES('"+uniqueid().trim()+"','"+county+"','"+district+"','"+id1+"','"+quarter+"','"+year+"','65')";  
}
...
//check whether there is an existing record then do an update                              
                ...
                if (conn.rs3.next()) {
                    ...
                    query = "insert into indicatorachieved(resultID,county,district,menAchieved,womenAchieved,reportingPeriod,financialYear,titleID)"
                            + "VALUES('"+uniqueid().trim()+"','" + county + "','" + district + "','" + id2 + "','" + id3 + "','" + quarter + "','" + year + "','21')";
                }
                try {
                    conn.state.executeUpdate(query);
                    replyback = "<font color=\"orange\"><b>Importing completed ";
            ...
            if(id4>-1){
String query="";                            
String checkexistingval="Select * from indicatorachievedcombined where  district='"+district+"' and financialYear='"+year+"' and titleID='64' and reportingPeriod='"+quarter+"' ";                              
conn.rs3=conn.state3.executeQuery(checkexistingval);
...
if(conn.rs3.next()){
  query = "update indicatorachievedcombined set totalAchieved='"+id4+"' where resultID='"+conn.rs3.getString(1)+"'";
    ...
    query = "insert into indicatorachievedcombined(resultID,county,district,totalAchieved,reportingPeriod,financialYear,titleID)"
                                   + "VALUES('"+uniqueid().trim()+"','"+county+"','"+district+"','"+id4+"','"+quarter+"','"+year+"','64')";  
}
...
if(conn.rs3.next()){
  query = "update indicatorachievedcombined set totalAchieved='"+id5+"' where resultID='"+conn.rs3.getString(1)+"'";
    ...
    query = "insert into indicatorachievedcombined(resultID,county,district,totalAchieved,reportingPeriod,financialYear,titleID)"
                                   + "VALUES('"+uniqueid().trim()+"','"+county+"','"+district+"','"+id5+"','"+quarter+"','"+year+"','65')";  
}
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(UploadHIVCumT14.class.getName()).log(Level.SEVERE, null, ex);
