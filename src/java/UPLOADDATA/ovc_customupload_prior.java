 ...
 * @author Maureen
 */
public class ovc_customupload_prior extends HttpServlet {
...
public void insertintoachievedcombined(  dbConnect conn, String year,String quarter, String county, String district, String indicator, int value, int rownum) throws SQLException{
    ...
    System.out.println("ACHIEVEDCOMBINED");
            ...
            if (value >-1) {
                String query = "";
                String checkexistingval = "Select * from indicatorachievedcombined_ovc_prior where  district='" + district + "' and financialYear='" + year + "' and titleID='"+indicator+"' and reportingPeriod='" + quarter + "'";
                conn.rs3 = conn.state3.executeQuery(checkexistingval);

                if (conn.rs3.next()) {
                    query = "update indicatorachievedcombined_ovc_prior set totalAchieved='" + value + "' where resultID='" + conn.rs3.getString(1) + "'";
...
//check whether there is an existing record then do an update                              
                String query = "";
                String checkexistingval = "Select * from indicatorachieved_ovc_prior where  district='" + district + "' and financialYear='" + year + "' and titleID='"+indicator+"' and reportingPeriod='" + quarter + "'";
                conn.rs3 = conn.state3.executeQuery(checkexistingval);

                if (conn.rs3.next()) {
                    query = "update indicatorachieved_ovc_prior set menAchieved='" + malevalue + "',womenAchieved='" + femalevalue + "' where resultID='" + conn.rs3.getString(1) + "'";
